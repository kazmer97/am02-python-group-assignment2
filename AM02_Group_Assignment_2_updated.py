...
tws1 = [p.clean(t) for t in twsUnclean]
...
tws3 = [x.strip().lower() for x in tws2]
...
tws4 = [x for x in tws3 if len(x) > 0]
...
'''
The average compound score is around 0.3. The tweets have a slightly positive sentiment.
The tweets seems to be mostly factual information on Tesla shares 
and people sharing tweets on their positive views or gains from Tesla shares.
'''
...
tws4 = [x for x in tws4 if x not in sw]
...
# b)
# words seperated out as elements
